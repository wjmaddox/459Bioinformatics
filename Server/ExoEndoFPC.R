...
save(fpc.score,file="~/EECS459/FullFPC.RData")
#plot(fpc.score)
...
fpc.validate <- function(assg){
  ...
  #dopar here
  fpc.valsT <- foreach(i=1:length(n),.combine=rbind,.export=c('fpc','nmc','semilc'),.packages='igraph') %dopar% (fpc(f.rem(assg,i)))
...
fpc.vals <- fpc.validate(exoEndo.graph)
fpc.val <- as.numeric(colSums(fpc.vals)/length(V(exoEndo.graph)))
save(fpc.val,file="~/EECS459/FullValFPC.RData")
